...
df = df[(df['Prix'] >= 50) & (df['Prix'] <= 3000)]  # Supprimer les lignes avec des prix en dehors de la plage (on ne remplace pas par NaN ici car c'est la variable que l'on veut estimer)
...
df.loc[(df['Taille écran'] < 6) | (df['Taille écran'] > 20), 'Taille écran'] = np.nan  # On limite la taille de l'écran à 20 pouces, seulement 3 valeurs au delà
